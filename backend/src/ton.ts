...
function client() {
  return new TonClient({ endpoint: env.TONCENTER_ENDPOINT, apiKey: env.TONCENTER_API_KEY || undefined });
}
...
export async function sendVton(recipient: string, rawAmount: string) {
  ...
  const master = ton.open(JettonMaster.create(Address.parse(env.VTON_JETTON_MASTER)));
  const treasuryJettonWallet = await master.getWalletAddress(wallet.address);
  ...
  await walletContract.sendTransfer({
    ...
    messages: [internal({ to: treasuryJettonWallet, value: toNano('0.05'), body: transferBody })]
  });
  return { seqno, treasury: wallet.address.toString({ urlSafe: true, bounceable: true }) };
}
